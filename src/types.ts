...
export type SetFieldValue<T = unknown> = (
  field: string,
  value: any,
...
export type Organization = {
  ...
  logo: string;
  name: string;
  settings: any; // Can be updated to a more specific type if the settings structure is known
  slug: string;
  updated: string;
  user: string;
  description: string;
...
export type EventTypeSettings = {
  ...
  user: string;
  settings: EventType;
  // member is an invite model not organization_membership model
  member: string;
};
...
export type Booking = {
  attendees: Array<{ email: string; name: string; host: boolean }>;
  cancelReason: string;
  collectionId: string;
  collectionName: string;
  created: string;
