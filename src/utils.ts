...
export const getImage = ({ imageName, collectionName, recordId }: { imageName: string; collectionName: string; recordId: string }) => {
...
export const flattenPocketBaseData = <T>(data: ExpandedRecord | ExpandedRecord[] | null): T | T[] => {
  if (!data) return [] as T[]
  ...
  const flatten = (item: ExpandedRecord): any => {
    ...
    if (result.expand) {
      Object.entries(result.expand).forEach(([key, value]) => {
        result[key] = Array.isArray(value) ? value.map((v) => flatten(v)) : value && typeof value === 'object' ? flatten(value) : value
      })
  ...
  return Array.isArray(data) ? data.map((item) => flatten(item)) : flatten(data)
}

export const browserTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone

export const formatToTimeZone = (time: string, timezone?: string | undefined, formatType?: string) => {
  if (!time) return ''
