...
export const SearchableSelector = ({ value, onChange, options, label, placeholder, showSuffix, selectorStyle, disabled, showBorder }: Props) => {
  ...
  return (
    <Box width="100%">
      ...
      >
        {/* @ts-ignore */}
        <CenterRow width="100%" justifyContent="space-between" {...selectorStyle} borderColor={showBorder ? 'border' : selectorStyle?.borderColor}>
              ...
              filteredOptions.map((option) => (
                ...
                >
                  <Text variant="light" style={{ wordWrap: 'break-word' }}>
                    {option.label ?? option.value} {showSuffix && showSuffix(option.value)}
                  </Text>
                </Box>
              ))
            ) : (
